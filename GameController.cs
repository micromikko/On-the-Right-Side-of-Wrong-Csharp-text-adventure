...
public class GameController : MonoBehaviour
{
	...
	{
		// INSTRUCTIONS FOR DEFINING NODES

		// 		// exampleNode												// name of node in a comment (for clarity).
		// 		nn_exampleNode.Add(nodeAfterExample1);						// node added to exampleNode's next node list. min 1, max 4.
		//		exampleNode.AddNextNodes(nn_exampleNode);					// link next node list to exampleNode.
		//
		// 		exampleNode.SetDialogue(d_exampleNode);						// link dialogue to exampleNode.
		//
		// 		exampleNode.SetHasClue(true/false);							// if exampleNode has a clue, set to true. Otherwise this line is not needed.
		//		exampleNode.SetClue("string");								// if exampleNode has a clue, write it here.
		//
		// 		exampleNode.SetHasAudioClip(true/false);					// if exampleNode has an audio clip, set to true. Otherwise this line is not needed.
		//		exampleNode.SetMyAudioClip(a_exampleAudio);					// if exampleNode has an audio clip, set to wanted audio clip's name.
		//
		// 		exampleNode.SetHasMusicClip(true/false);					// if exampleNode has a music clip, set to true. Otherwise this line is not needed..
		//		exampleNode.SetMyMusicClip(m_exampleMusic);					// if exampleNode has a music clip, set to wanted music clip's name.
		//
		//
		// 		beforeExampleStore.SetNextIsStore(true/false);				// node before exampleStore. Must have beforeExampleStore for store to work. Set to true. Otherwise this line is not needed. Can have only 1 next node. 
		// 		exampleStore.SetStoreItems(buttonX, exampleItem);			// link items to buttons and add key value pair to exampleStore. min 1, max 4.
		//		exampleStore.SetNextIsReceipt(true/false);					// exampleStore must have an exampleStoreReceipt node. Set to true.
		//
		// 																	// additional store rules:
		//																	// exampleStore must have as many next nodes as it as items. All next nodes have to be exampleStoreReceipt
		//																	// exampleStoreReceipt dialogue must be set to d_receipt
		//																	// exampleStore should have at least 1 option that costs nothing. Otherwise the player may find theirselves unable to progress.
	...
	private void CreateUI()
	{
		...
		*/
		...
		buttonResume.onClick.AddListener (() => (ResumeGame() ));							//listeners for menu buttons
		buttonMainMenu.onClick.AddListener (() => (DisplayMainMenu() ));
		buttonStartNewGame.onClick.AddListener (() => (StartNewGame () ));
		buttonQuit.onClick.AddListener (() => (QuitGame ()));
		
		button1.onClick.AddListener (() => (ButtonClicked (button1)));						//listeners for gameplay buttons
		button2.onClick.AddListener (() => (ButtonClicked (button2)));
		button3.onClick.AddListener (() => (ButtonClicked (button3)));
		button4.onClick.AddListener (() => (ButtonClicked (button4)));
		
		buttonClueList.onClick.AddListener (() => (ToggleClueListAndCash () ) );			//listener for cluelist+cash toggle
	}
	...
	private void NodeNormal(Button b)
	{
		...
		if (currentNode.GetNextIsReceipt() == true){    //if the player came from a store node, subtract price of chosen item from player's funds
		...
		if(currentNode.GetHasClue() == true){						//write to cluelist
	...
	private IEnumerator TypeTextAndEnableButtonNormal(string textToPrint) //takes the dialogue string from currentNode as a parameter
	{
		...
		foreach (char letter in textToPrint.ToCharArray()) {		//loops through the string and prints out one character on each iteration
			//Debug.Log(letter);
			this.dialogueBox.text += letter;
			yield return new WaitForSeconds(letterPause);
		}
		switch(currentNode.GetNextNodes().Count){					//activate as many buttons as there are nextNodes in the list
	...
	private IEnumerator TypeTextAndEnableButtonStore(string textToPrint)	//separate method for printing text in a store node
	{
		...
		foreach(KeyValuePair<Button, Item> _item in currentNode.GetDic() ){	//get the product catalog (Dictionary) from the store node
			if (_item.Value.GetItemPrice() <= steele.GetCash()){			//activate buttons only for items that the player can afford
	...
	private void WriteToClueList() {										//write the last entry on the cluelist to screen
